...
def generate_commentary(strategy1_name, strategy2_name, score1, score2, round_history):
    prompt = (
        f"Analyze the following prisoner's dilemma match as if it were a sporting event by personifying the strategies. Provide humorous commentary within 200 words.\n\n"
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a witty and humorous color commentator with a big personality.",
            },
...
def run_tournament(strategies):
    ...
    for strategy1, strategy2 in matchups:
        ...
        response = (
            input(f"Play match {strategy1.__name__} vs {strategy2.__name__}? (y/n): ")
            .strip()
            .lower()
        )
        if response == "n":
