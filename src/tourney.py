...
def generate_commentary(strategy1_name, strategy2_name, score1, score2, round_history):
    prompt = (f"Analyze the following match as if it were a sporting event. Provide a dramatic commentary within 100 words.\n\n"
...
def generate_final_commentary(sorted_results):
    ...
    for strategy, data in sorted_results:
        ...
        prompt += (f"{strategy}: Total Score = {total_score}, Avg Score = {avg_score:.2f}, "
...
def run_tournament(strategies):
    ...
    matchups = [(strategies[i], strategies[j]) for i in range(len(strategies)) for j in range(i + 1, len(strategies))]
    ...
    for strategy1, strategy2 in matchups:
        ...
        if not skip_prompt:
            ...
            print("3) Run the tournament to completion without commentary or further prompting between matches")
        ...
        score1, score2, new_reputation1, new_reputation2, round_history = play_match(strategy1, strategy2, rounds, reputation1, reputation2, 0.1)
        ...
        if commentary_enabled and not skip_prompt:
            commentary = generate_commentary(strategy1.__name__, strategy2.__name__, score1, score2, round_history)
            print(f"\nGPT Commentary:\n{commentary}\n")
